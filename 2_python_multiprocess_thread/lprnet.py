...
class LPRNet(object):
    def __init__(self, lprnet_bmodel,dev_id):
        # load bmodel
        self.net = sail.Engine(lprnet_bmodel, dev_id, sail.IOMode.SYSO)
        logging.debug("load {} success!".format(lprnet_bmodel))
    ...
    def preprocess_bmcv(self, input_bmimg, output_bmimg):
        ...
        logging.info('LPRNET PREPROCESS DONE')
        return output_bmimg
    ...
    def predict(self, input_tensor):
        ...
        logging.info('LPRNET PROCESS DONE')
        return outputs
    ...
    def postprocess(self, outputs):
        ...
        logging.info('LPRNET POSTPROCESS DONE,len(CHARS) {}'.format(len(CHARS)))
        return res
    ...
    def __call__(self, bmimg_list):
        ...
        if self.batch_size == 1:
            ...
            # for bmimg in bmimg_list:
                # logging.debug(f"LPRNET IS 1 BATCH, input image is{img_num}")
            ...
            start_time = time.time()
            res.append(self.postprocess(outputs))
            ...
            logging.debug("LPRNET IS 4 BATCH")
            BMImageArray = eval("sail.BMImageArray{},img num is{}".format(self.batch_size,img_num))
            bmimgs = BMImageArray()
...
def main(args):
    ...
    if len(bmimg_list):
        res_list = lprnet(bmimg_list) # bmimg的list传入lprnet，然后得到结果
        for i, filename in enumerate(filename_list):
            logging.info("filename: {}, res: {}".format(filename, res_list[i]))
            res_dict[filename] = res_list[i]
