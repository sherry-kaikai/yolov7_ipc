...
class Resnet_Engine(object):
    ...
    def engine_process(self, image_pipe_decode2engine, dist_pipe_decode2engine, image_pipe_engine2post, dist_pipe_engine2post):
        ...
        while True:
            ...
            # print("111111111111",len(output_tensor_map)) # 1
            ...
            # print("22222222222",tensor.shape()) # [1, 1000]
            ipc_engine2post.sendTensor(tensor, channel_id, frame_id) # 传递tensor 失败了 IPC需要改

    def postprocess_(self, outputs):
        res = list()
...
def process(resnet,image_pipe_decode2engine, dist_pipe_decode2engine, image_pipe_engine2post, dist_pipe_engine2post):
    resnet.engine_process(image_pipe_decode2engine, dist_pipe_decode2engine, image_pipe_engine2post, dist_pipe_engine2post)
    

def resnet_post_process(resnet, image_pipe_engine2post, dist_pipe_engine2post):
    resnet.postprocess(image_pipe_engine2post, dist_pipe_engine2post)
...
def main(args):
    ...
    这样是不行的，传class的方法进去没有被实例化。无法在多进程中传递。

    '''
    decode2engine_processes = [multiprocessing.Process(target=process, args=(resnet_engine, image_pipe_decode2engine, dist_pipe_decode2engine, image_pipe_engine2post, dist_pipe_engine2post)) for i in range(proesss_nums)]

    '''
    2 把后处理扔到进程里面
    '''
    post_processes = [multiprocessing.Process(target=resnet_post_process, args=(resnet_engine,image_pipe_engine2post, dist_pipe_engine2post)) for i in range(proesss_nums)]
    ...
    while True:
        try:
            ...
            for p in decode2engine_processes:
                p.terminate
...
if __name__ == '__main__':
    ...
    logging.basicConfig(filename= f'process_is_{name_number}.log', level=logging.DEBUG)
    main(args)
