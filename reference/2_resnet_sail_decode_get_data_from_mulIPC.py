...
解码,并传入bmimage
'''
def decode_images(channel_id, input_path, image_pipe, dist_pipe, frame_flag):
    # print('---------------sender---------------------')
    ...
    if os.path.isfile(input_path): # 视频流
        ...
        while True:
            bmi = sail.BMImage()
            frame_id += 1
            ret = decoder.read(handle,bmi)
            if not ret: 
                ipc.sendBMImage(bmi, channel_id, frame_id)
            else: 
                time.sleep(0.001) # sleep并不会让程序work
                # print('************************read end ************************')
                frame_flag[channel_id] = 1 # 此进程读取结束 flag = 1
                break
...
def main(args):
    ...
    input_video = [args.input for _ in range(process_num)]
    frame_flag= multiprocessing.Array("i",[0 for _ in range(process_num)])

    image_pipe = "/tmp/img"
    dist_pipe = "/tmp/final"
    image_pipe_list =[ "/tmp/img"+str(i) for i in range(process_num) ]
    dist_pipe_list = ["/tmp/final"+str(i) for i in range(process_num) ]
    '''
    每一个视频流都有一个解码进程、一个单独的ipc管道和一个处理进程。
    '''
    send_processes = [multiprocessing.Process(target=decode_images, args=(cid, input_video[cid], image_pipe_list[cid], dist_pipe_list[cid], frame_flag)) for cid in range(process_num)]
    receive_processes = [multiprocessing.Process(target=process_images, args=( resnet, image_pipe_list[cid], dist_pipe_list[cid], frame_flag)) for cid in range(process_num)]
...
def argsparser():
    ...
    parser.add_argument('--process_num', type=int, default=3, help='path of input, must be image directory')
